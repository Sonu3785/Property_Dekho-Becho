...
export default function OwnerDashboard() {
  ...
  const sharedProps = { properties, tenants, payments, agreements, refresh: fetchAll, user }
  ...
  return (
    <div className={styles.dashboard}>
      ...
      <div className={styles.content}>
        ...
        {!loading && activeTab === 'overview'   && <Overview   {...sharedProps} setActiveTab={setActiveTab} />}
        {!loading && activeTab === 'properties' && <Properties {...sharedProps} />}
        {!loading && activeTab === 'tenants'    && <Tenants    {...sharedProps} />}
        {!loading && activeTab === 'agreements' && <Agreements {...sharedProps} />}
        {!loading && activeTab === 'payments'   && <Payments   {...sharedProps} />}
      </div>
    </div>
  )
}
    ...
    <div>
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.subtitle}>Here's your property portfolio summary</p>
        </div>
      </div>
      <div className={styles.statsGrid}>
        {[
          ...
          { icon: '✅', label: 'Paid',           value: paidCount,          color: '#10b981', tab: 'payments'   },
        ].map((s, i) => (
      ...
      <div className={styles.overviewGrid}>
        <div className={styles.overviewCard}>
          ...
          {properties.length === 0 && <p className={styles.empty}>No properties yet. Add one!</p>}
        </div>
        <div className={styles.overviewCard}>
          ...
          {tenants.slice(0, 5).map(t => (
            <div key={t.id} className={styles.listRow}>
              ...
              <div className={styles.listInfo}>
                ...
                <div className={styles.listSub}>{t.email} • {t.phone}</div>
              </div>
              <div className={styles.listBadge2}>Prop #{t.property_id}</div>
            </div>
          ))}
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      if (res.data.success !== false) {
        toast.success('Property added! 🏢'); refresh()
        setShowForm(false); setForm({ title: '', location: '', price: '' })
      } else toast.error(res.data.error || 'Failed to add')
  ...
  return (
    <div>
      <div className={styles.pageHeader}>
        <div>
          <h2>My Properties</h2>
          <p className={styles.subtitle}>{properties.length} total properties listed</p>
        </div>
      ...
      {showForm && (
        <div className={styles.formBox}>
          ...
          <form onSubmit={handleAdd} className={styles.inlineForm}>
            <input placeholder="Property Title (e.g. 2BHK Flat)" value={form.title}    onChange={e => setForm({...form, title: e.target.value})} required />
            <input placeholder="Location (e.g. Pune, MH)"         value={form.location} onChange={e => setForm({...form, location: e.target.value})} required />
            <input type="number" placeholder="Monthly Rent (₹)"   value={form.price}    onChange={e => setForm({...form, price: e.target.value})} required />
        ...
        {filtered.map(p => (
          <div key={p.id} className={styles.propCard}>
            <div className={styles.propCardHeader}>
              ...
              <button className={styles.deleteBtn} onClick={() => handleDelete(p.id)} title="Delete property">🗑️</button>
            </div>
            <h4>{p.title}</h4>
            <p className={styles.propLocation}>📍 {p.location}</p>
            <div className={styles.propPrice}>₹{p.price?.toLocaleString()}<span>/month</span></div>
            <div className={styles.propMeta}>Property ID: #{p.id}</div>
          </div>
        ))}
        {filtered.length === 0 && <p className={styles.empty}>No properties found. Add your first property above!</p>}
      </div>
    </div>
  )
}
...
function Tenants({ tenants, properties, refresh }) {
  ...
  const filtered = tenants
    ...
    .sort((a, b) => {
      let av = String(a[sortBy] || '').toLowerCase()
      let bv = String(b[sortBy] || '').toLowerCase()
      return sortDir === 'asc' ? (av > bv ? 1 : -1) : (av < bv ? 1 : -1)
    })
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      toast.success('Tenant added! 👤'); refresh()
      setShowForm(false); setForm({ name: '', phone: '', email: '', property_id: '' })
  ...
  return (
    <div>
      ...
      <div className={styles.toolbar}>
        <input className={styles.search} placeholder="🔍 Search by name, email or phone..." value={search} onChange={e => setSearch(e.target.value)} />
        <select className={styles.select} value={sortBy} onChange={e => setSortBy(e.target.value)}>
            ...
            {filtered.map(t => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td>{t.phone}</td>
                  <td>{t.email}</td>
                  <td><span className={styles.badge}>{prop ? `${prop.title}` : `Property #${t.property_id}`}</span></td>
                </tr>
              )
            })}
          </tbody>
...
function Agreements({ properties, tenants, agreements, refresh }) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      toast.success('Agreement created! 📄'); refresh()
      setShowForm(false); setForm({ tenant_id: '', property_id: '', start_date: '', end_date: '', rent: '', deposit: '' })
    } catch { toast.error('Failed to create agreement') }
    finally { setSaving(false) }
  }
    ...
    <div>
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.subtitle}>{agreements.length} total agreements</p>
        </div>
        <button className={styles.addBtn} onClick={() => setShowForm(!showForm)}>
      ...
      <div className={styles.tableWrapper}>
        <table className={styles.table}>
          <thead>
            <tr><th>#</th><th>Tenant</th><th>Property</th><th>Start</th><th>End</th><th>Rent</th><th>Deposit</th></tr>
          </thead>
          <tbody>
            {agreements.map((ag, i) => {
              ...
              return (
                <tr key={ag.id}>
                  ...
                  <td>{p?.title || `Property #${ag.property_id}`}</td>
                  <td>{ag.start_date}</td>
                  <td>{ag.end_date}</td>
        ...
        {agreements.length === 0 && <p className={styles.empty}>No agreements yet. Create one above!</p>}
      </div>
    </div>
  )
}
...
function Payments({ payments, tenants, refresh }) {
  ...
  const filtered = payments.filter(p => {
    ...
    const matchSearch = String(p.tenant_id).includes(search) ||
      t?.name?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      toast.success('Payment recorded! 💳'); refresh()
      setShowForm(false); setForm({ tenant_id: '', amount: '', date: '', status: 'paid' })
  ...
  return (
    <div>
      ...
      <div className={styles.toolbar}>
        <input className={styles.search} placeholder="🔍 Search by tenant name or status..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <tbody>
            {filtered.map((p, i) => {
              ...
              return (
                <tr key={p.id}>
                  <td>{i + 1}</td>
                  <td><div className={styles.tenantName}><div className={styles.miniAvatar}>{t?.name?.[0]?.toUpperCase() || '?'}</div>{t?.name || `Tenant #${p.tenant_id}`}</div></td>
